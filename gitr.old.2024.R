...
gitr = function(probes, phenos = TRUE, nonormal = TRUE, noheme = FALSE,
    cohort = 'all', conn = con,
    makefactors = TRUE,
    db = 'tcga',
    dbcat = 'tcgacat'
    ...
    ## why would I have needed to remove muttest.muttest?
    #######if(MUTTEST){ out = out %>% dplyr::select( -muttest.muttest ) }
    ##print(out)
