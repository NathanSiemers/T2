...
shinyUI(
    fluidPage(
        ...
        h5( Sys.Date() ),
        tags$br(),
        h4('Conditioning Variables (not operational yet)'),
        selectizeInput('condition', 'Conditioning against:', choices = NULL, multiple = TRUE ),
        div(style="display:inline-block;", checkboxInput("fullsemi", "Full Partial Correlation?", value = FALSE)),
        tags$br(),
        h4('Fiddly Options'),
